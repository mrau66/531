...
class UnifiedStateStore {
    ...
    async handleUserSignIn(user) {
        ...
        // "We have local changes IF:
        // The user has made at least one change locally, AND
        // EITHER we've never synced to the database before, OR the local change is newer than our last sync"
        const hasLocalChanges = this.state.lastLocalChange && 
            (!this.state.lastDatabaseSync || 
            new Date(this.state.lastLocalChange) > new Date(this.state.lastDatabaseSync));
        
        if (hasLocalChanges) {
            console.log('⚠️ Local changes detected, syncing to database instead of loading');
    ...
    setAllTrainingMaxes(maxes) {
        this.updateState({ trainingMaxes: { ...this.state.trainingMaxes, ...maxes } });
    }
    ...
    setSessionCompletion(lift, cycle, week, data) {
        ...
        if (this.state.isInitialLoadComplete) {
            this.updateState({
                sessionCompletion: { ...this.state.sessionCompletion, [key]: validatedData }
            });
        } else {
    ...
    async loadAccessories() {
        ...
        if (data?.accessories) {
            ...
            this.updateState({
                accessories: {
                    squat: ["Bulgarian Split Squats - 3x8-12 each leg", "Jump Squats - 3x8-10"],
                    bench: ["Push-ups (various grips) - 3x8-15", "Tricep Dips - 3x8-12"],
                    deadlift: ["Single-leg Romanian Deadlifts - 3x8-10 each", "Glute Bridges - 3x12-15"],
    ...
    async saveAllSessionCompletion() {
        const promises = Object.entries(this.state.sessionCompletion).map(([key, data]) => {
    ...
    async saveTrainingMaxesToDB() { return this.saveTrainingMaxes(); }
    async saveCycleSettingsToDB() { return this.saveCycleSettings(); }
    async saveAccessoriesToDB() { return this.saveAccessories(); }
    ...
    createSyncUI() {
        ...
        const inputSection = document.querySelector('.input-section .section-header');
    ...
    generateWorkout() {
        console.warn('generateWorkout() is deprecated');
    }
    
    refreshCycleInfo() {
        console.warn('refreshCycleInfo() is deprecated');
    }
}
