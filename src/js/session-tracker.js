...
class SessionTracker {
    ...
    async init() {
        ...
        this.dbEnabled = !!(window.auth?.currentUser && window.supabase && window.stateStore);
        
        // Load states when ready
        if (window.stateStore?.getState('isInitialLoadComplete')) {
            console.log('✅ StateStore ready, loading states');
            ...
            if (this.dbEnabled) {
                await this.waitForDatabaseLoad();
                await this.loadAllStatesFromStateStore();
            } else {
        ...
        if (window.stateStore) {
            window.stateStore.subscribe('cycleSettings', () => {
                ...
                this.reloadTimeout = setTimeout(() => this.loadAllStatesFromStateStore(), 200);
            });
            
            if (!this.isInitialized) {
                window.stateStore.subscribe('isInitialLoadComplete', (isComplete) => {
                    if (isComplete && !this.isInitialized) {
    ...
    handlePointerDown(e) {
        ...
        if (target.classList.contains('set-row') || target.classList.contains('accessory-item')) {
            const inWorkoutArea = target.closest('.workout-display, .lift-workout, .tab-content, [data-tab]');
            if (!inWorkoutArea) return;
            
            // Prevent double processing with element-specific check
            const elementId = `${target.className}-${Array.from(target.parentNode.children).indexOf(target)}`;
            const now = Date.now();
            
            if (this.lastProcessed === elementId && (now - this.lastProcessedTime) < 500) {
                return;
            }
    ...
    async waitForDatabaseLoad() {
        ...
        let retries = 0;
        while (retries < 100 && !window.stateStore.getState('isInitialLoadComplete')) {
            await new Promise(resolve => setTimeout(resolve, 100));
            retries++;
        }
    }
    ...
    initializeCompletionArrays(liftType) {
        ...
        const mainSetCount = container.querySelectorAll(`#${liftType}-main-sets .set-row:not([style*="display: none"])`).length;
        const supplementalSetCount = container.querySelectorAll(`#${liftType}-supplemental-sets .set-row:not([style*="display: none"])`).length;
        ...
        const updatedCompletion = {
            mainSets: currentCompletion.mainSets.length === mainSetCount ? 
                      currentCompletion.mainSets : new Array(mainSetCount).fill(false),
            supplementalSets: currentCompletion.supplementalSets.length === supplementalSetCount ? 
                             currentCompletion.supplementalSets : new Array(supplementalSetCount).fill(false),
            accessories: currentCompletion.accessories.length === accessoryCount ? 
                        currentCompletion.accessories : new Array(accessoryCount).fill(false)
        };
        
        if (JSON.stringify(currentCompletion) !== JSON.stringify(updatedCompletion)) {
    ...
    updateProgress(liftType) {
        ...
        const completed = state.mainSets.filter(Boolean).length + 
                         state.supplementalSets.filter(Boolean).length + 
                         state.accessories.filter(Boolean).length;
        const total = state.mainSets.length + state.supplementalSets.length + state.accessories.length;
    ...
    save = this.debounce(async (liftType) => {
        ...
        try {
            const { cycle, week } = window.stateStore.getCycleSettings();
            const completionData = window.stateStore.getSessionCompletion(liftType, cycle, week);
            await window.stateStore.saveSessionCompletionToDB(liftType, completionData);
        } catch (error) {
    ...
    loadFromLocalStorage() {
        setTimeout(() => this.applyAllStatesToUI(), 100);
    }
    ...
    getSessionSummary() {
        ...
        ['squat', 'bench', 'deadlift', 'ohp'].forEach(lift => {
            ...
            if (state) {
                ...
                const liftCompleted = state.mainSets.filter(Boolean).length + 
                                   state.supplementalSets.filter(Boolean).length + 
                                   state.accessories.filter(Boolean).length;
...
document.addEventListener('DOMContentLoaded', function() {
    const hasWorkoutElements = document.getElementById('workout-display') || 
